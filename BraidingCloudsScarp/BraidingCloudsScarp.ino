...
//  modulation / input vars for modules
...
#define TIMER0_INTERVAL_MS 31.25 // 32000 \20.833333333333running at 48Khz
...
void updateControl() {
  ...
  if ( display_mode == 0) {
    if (! potlock[0] ) {
      int p1 = potvalue[0];
      morph_in = (float)p1 / 4095.0f;
      braids_morph = constrain(morph_in, 0.0f, 1.0f);
    }
    if (! potlock[1] ) {
      int p2 = potvalue[1];
      timbre_in = (float)p2 / 4095.0f;
      braids_timbre = constrain(timbre_in, 0.0f, 1.0f);
    }
    ...
    if (! potlock[0] ) {
      int p1 = potvalue[0];
      morph_in = (float)p1 / 4095.0f;
      clouds_morph = constrain(morph_in, 0.0f, 1.0f);
    }
    if (! potlock[1] ) {
      int p2 = potvalue[1];
      timbre_in = (float)p2 / 4095.0f;
      clouds_timbre = constrain(timbre_in, 0.0f, 1.0f);
    }
  } else if (display_mode == 2) {
    if (! potlock[0] ) {
      int p1 = potvalue[0];
      morph_in = (float)p1 / 4095.0f;
      clouds_reverb = constrain(morph_in, 0.0f, 1.0f);
    }
    if (! potlock[1] ) {
      int p2 = potvalue[1];
      timbre_in = (float)p2 / 4095.0f;
      clouds_fb = constrain(timbre_in, 0.0f, 1.0f);
    }
  }
  scanbuttons();
...
void loop1() {
  ...
  // increment mode/program for braids or clouds
  if (  encoder_delta != 0 && encoder_held) {
    if (display_mode == 0) {
      engineCount = braids_engine + encoder_delta;
      braids_engine = constrain(engineCount, 0, 47); // ( engine +) % voices[0].voice_.GetNumEngines();
    } else {
      engineCount = clouds_engine + encoder_delta;
      clouds_engine = constrain(engineCount, 0, 3); // ( engine +) % voices[0].voice_.GetNumEngines();
    }
  }
  ...
  if (! anybuttonpressed && encoder_delta && ! encoder_held) {
    ...
    if (display_mode > 0 ) {
      clouds_harm = constrain(clouds_harm + turn, 0.0, 1.0);
      harm_in = clouds_harm;
    } else {
      clouds_pos = constrain(clouds_pos + turn, 0.0, 1.0);
      harm_in = clouds_pos;
    }
