...
void updateCloudsAudio() {
  ...
  float   reverb = constrain(clouds_reverb, 0.f, 0.9f); // IN0(8);
  float   fb =   constrain(clouds_fb, 0.f, 0.9f); // IN0(9);
  float   tex = constrain (clouds_timbre, 0.f, 0.8f) ;

  float   posi = constrain(clouds_pos, 0.f, 0.9f); //constrain(pos_mod, 0.f, 1.f);

  float   siz = constrain(clouds_harm, 0.f, 0.7f) ;// 0.35f;
  
  float   dens = constrain(clouds_morph, 0.f, 0.7f);;

  float   drywet = 1.0; // constrain(clouds_dw_in, 0.3f, 1.0f);
  bool    freeze = freeze_in;

  short   engine = constrain(clouds_engine, 0, 3); // 0 -3
  ...
  */
  ...
  for (int i = 0; i < cloudsAudioBlockSize; ++i) {
    out_bufferL[i] = stmlib::Clip16( static_cast<int32_t>( (  output[i].l + output[i].r)  * 32768.0f  ) ) ; // static 2 gain
